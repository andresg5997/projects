...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_vue_multiselect__ = __webpack_require__(155);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_vue_multiselect___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2_vue_multiselect__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_toastr__ = __webpack_require__(32);
...
var app = new __WEBPACK_IMPORTED_MODULE_0_vue___default.a({
	...
	methods: {
		...
		botonEdit: function botonEdit(index) {
			var _this = this;
			...
			this.estados.forEach(function (estado) {
				if (estado.id == _this.form.estado_id) {
					_this.selectedEstadoEdit = estado;
					_this.procesos.forEach(function (proceso) {
						if (estado.proceso_id == proceso.id) {
							_this.selectedProcesoEdit = proceso;
						}
					});
				}
			});
			this.estados.forEach(function (estado) {
				if (estado.proceso_id == _this.selectedProcesoEdit.id) {
					_this.procesoEstadosEdit.push(estado);
				}
			});
			$('#dashboardEditModal').modal('open');
		},
		botonAdd: function botonAdd() {
			if (this.dashboards.length == 4) {
				__WEBPACK_IMPORTED_MODULE_3_toastr___default.a.error('No pueden haber más de 4 paneles.');
			} else {
				this.form = {
					estado_id: '',
					dias_estado: 0
		...
		selectProceso: function selectProceso(event) {
			...
			this.estados.forEach(function (estado) {
				if (estado.proceso_id == event.id) {
					_this2.procesoEstados.push(estado);
				}
			});
		},
		selectEstado: function selectEstado(event) {
			this.form.estado_id = event.id;
		},
		editProceso: function editProceso(event) {
			var _this3 = this;
			...
			this.estados.forEach(function (estado) {
				if (estado.proceso_id == event.id) {
					_this3.procesoEstadosEdit.push(estado);
				}
			});
		},
		quitarPanel: function quitarPanel(index) {
			var _this4 = this;
			...
			__WEBPACK_IMPORTED_MODULE_4_sweetalert2___default()({
				...
				__WEBPACK_IMPORTED_MODULE_1_axios___default.a.delete(laroute.route('dashboard.destroy', { dashboard: _this4.dashboards[index].id })).then(function (res) {
					if (res.data.saved) {
						__WEBPACK_IMPORTED_MODULE_3_toastr___default.a.info('El panel se ha eliminado.');
						_this4.dashboards.splice(index, 1);
					}
				});
			});
		},
		editPanel: function editPanel() {
			...
			__WEBPACK_IMPORTED_MODULE_1_axios___default.a.put(laroute.route('dashboard.update', { dashboard: this.dashboards[this.editIndex].id }), this.form).then(function (res) {
				if (res.data.updated) {
					$('#dashboardEditModal').modal('close');
					__WEBPACK_IMPORTED_MODULE_3_toastr___default.a.success('¡El panel fue editado con éxito!');
					_this5.dashboards[_this5.editIndex] = res.data.dashboard;
					_this5.form.estado = '';
					_this5.form.dias_estado = 0;
				}
			});
		},
		submit: function submit() {
			...
			__WEBPACK_IMPORTED_MODULE_1_axios___default.a.post(laroute.route('dashboard.store'), this.form).then(function (res) {
				if (res.data.saved) {
					__WEBPACK_IMPORTED_MODULE_3_toastr___default.a.success('¡El panel ha sido agregado con éxito!');
					$('#dashboardModal').modal('close');
					var dashboardTemporal = res.data.dashboard;
					_this6.estados.forEach(function (estado) {
						if (_this6.form.estado_id == estado.id) {
							dashboardTemporal.estado = estado;
						}
					});
					console.log(dashboardTemporal);
					_this6.dashboards.push(dashboardTemporal);
					_this6.procesoEstados = [];
					_this6.selectedProceso = '';
					_this6.form.estado = '';
					_this6.form.dias_estado = 0;
				}
			});
		}
	}
