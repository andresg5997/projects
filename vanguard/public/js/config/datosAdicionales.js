...
var app = new __WEBPACK_IMPORTED_MODULE_0_vue___default.a({
    ...
    data: {
        datosAdicionales: [],
        opciones: [{ categoria: 'Teléfono', valor: 'telefono' }, { categoria: 'Dirección', valor: 'direccion' }, { categoria: 'Otro', valor: 'otro' }],
        selectedCategorias: [],
    ...
    methods: {
            ...
            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('marcas.datos.config')).then(function (res) {
                res.data.datosAdicionales.forEach(function (dato) {
                    if (dato.cliente_id == -1) {
                        ...
                        var categoriaObj = {
                            categoria: _this.humanize(dato.categoria)
                        };
                        _this.datosAdicionales.push(obj);
                        _this.selectedCategorias.push(categoriaObj);
                    }
                });
            });
        },
        selectCategoria: function selectCategoria(event, index) {
            this.datosAdicionales[index].categoria = event.valor;
        },
        agregarDato: function agregarDato() {
            var obj = {
                categoria: '',
                nombre: ''
            };
        ...
        borrarDato: function borrarDato(index) {
        ...
        submit: function submit() {
            ...
            this.errors = [];
            this.error = false;
            this.datosAdicionales.forEach(function (dato) {
                if (!dato.categoria || !dato.nombre) {
                    _this2.errors[index] = true;
                    _this2.error = true;
                }
                index++;
            });
            if (!this.error) {
                __WEBPACK_IMPORTED_MODULE_1_axios___default.a.post(laroute.route('marcas.datos.config.store'), this.datosAdicionales).then(function (res) {
                    if (res.data.updated) {
                        _this2.showConfirmacion = true;
                        setTimeout(function () {
