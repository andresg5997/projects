...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_toastr__ = __webpack_require__(32);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_vue_multiselect__ = __webpack_require__(155);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_sweetalert2__ = __webpack_require__(156);
...
new __WEBPACK_IMPORTED_MODULE_0_vue___default.a({
    ...
    data: {
        ...
        form: {
            ...
            estado_posterior: [],
            requisitos: [{ nombre: '', tipo: '', opciones: [], index: '' }],
            tiempo_seguimiento: '',
            titulo_tarea: '',
            tareas: [{ titulo: '' }]
        },
        tipos: [{ nombre: 'Texto', tipo: 'text', index: 0 }, { nombre: 'Fecha', tipo: 'date', index: 1 }, { nombre: 'Archivo', tipo: 'file', index: 2 }, { nombre: 'Seleccion', tipo: 'select', index: 3 }, { nombre: 'Mapa', tipo: 'map', index: 4 }, { nombre: 'Automatizada', tipo: 'auto', index: 5 }, { nombre: 'Campo de cliente', tipo: 'cliente', index: 6 }],
        tipos_campo_cliente: [{ nombre: 'Nombre', tipo: 'nombre' }, { nombre: 'Apellido', tipo: 'apellido' }, { nombre: 'Correo Electrónico', tipo: 'email' }, { nombre: 'Ciudad', tipo: 'ciudad' }, { nombre: 'País', tipo: 'pais' }, { nombre: 'Dirección', tipo: 'direccion' }],
        tipos_auto: [{ nombre: 'Correo Electronico', tipo: 'email', tipo_tarea: '', tipo_fecha: '', fecha: '' }, { nombre: 'Mensaje de Texto', tipo: 'sms', tipo_tarea: '', tipo_fecha: '', fecha: '' }],
        tipo_fecha_auto: [{ nombre: 'Al hacer la tarea', tipo: 'tarea' }, { nombre: 'Seleccionar tiempo', tipo: 'fecha' }],
        tipo_tarea_email: [],
        tipo_tarea_sms: [],
        procesos: [],
        estados: [],
    ...
    methods: {
        agregarRequisito: function agregarRequisito() {
        ...
        quitarRequisito: function quitarRequisito(index) {
            if (this.form.requisitos.length > 1) {
                this.form.requisitos.splice(index, 1);
                return;
            }
            __WEBPACK_IMPORTED_MODULE_2_toastr___default.a.error('Debe haber al menos un requisito.');
        },
        ...
        quitarOpcion: function quitarOpcion(index, indexOpc) {
            this.form.requisitos[index].opciones.splice(indexOpc, 1);
        },
        store: function store() {
            var _this = this;

            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.put(laroute.route('estados.update', { estado: this.id }), this.form).then(function (res) {
                if (res.data.updated) {
                    __WEBPACK_IMPORTED_MODULE_4_sweetalert2___default()('El estado se ha actualizado con éxito!', '', 'success');
                    setTimeout(function () {
                        window.location = laroute.route('procesos.show', { proceso: _this.form.proceso_id });
                    }, 1500);
                    return;
                }
            }).catch(function (error) {
        ...
        getEstados: function getEstados() {
            ...
            this.estados = [];
            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('api.estados')).then(function (res) {
                res.data.estados.forEach(function (estado) {
                    if (_this2.form.proceso.id == estado.proceso_id) {
                        var obj = {
                            nombre: estado.nombre,
                            id: estado.id
                        };
                        _this2.estados.push(obj);
                    }
                });
            });
        },
        getData: function getData() {
            ...
            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('api.procesos')).then(function (res) {
                _this3.procesos = res.data.procesos;
            });
            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('api.correos')).then(function (res) {
                res.data.correos.forEach(function (correo) {
                    var obj = {
                        id: correo.id,
                        correo_destino: correo.correo_destino,
                        id_plantilla: correo.id_plantilla
                    };
                    _this3.tipo_tarea_email.push(obj);
                });
            });
            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('api.estados')).then(function (res) {
                res.data.estados.forEach(function (estado) {
                    if (_this3.form.proceso_id == estado.proceso_id) {
                        var obj = {
                            nombre: estado.nombre,
                            id: estado.id
                        };
                        _this3.estados.push(obj);
                    }
                });
            });
        },
        getEstado: function getEstado() {
            ...
            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('estado.get', { estado: this.id })).then(function (res) {
                _this4.estado = res.data.estado;
                _this4.estado_anterior = res.data.estado_anterior;
                _this4.form = res.data.estado;
                _this4.form.estado_posterior = _this4.parseEstados(storage.estado_posterior);
                _this4.form.requisitos = _this4.parseRequisitos(_this4.estado.requisitos);
                _this4.form.proceso = _this4.procesos.filter(function (proceso) {
                    return proceso.id === parseInt(_this4.form.proceso_id);
                })[0];
            });
        },
        cambiarTipo: function cambiarTipo(index, event) {
            this.form.requisitos[index].tipo = event.tipo;
            this.form.requisitos[index].index = event.index;
        },
        selectFecha: function selectFecha(index, event) {
            this.form.requisitos[index].opciones.tipo_fecha = event.tipo;
        },
        selectCorreo: function selectCorreo(index, event) {
            var obj = {
                id: event.id,
        ...
        parseEstados: function parseEstados(ids) {
            var _this5 = this;

            var idsArray = ids.split(',');
            var results = [];
            idsArray.forEach(function (id) {
                _this5.estados.forEach(function (estado) {
                    if (estado.id == id) {
                        var obj = estado;
                        if (obj != undefined) {
                            results.push(obj);
        ...
        parseRequisitos: function parseRequisitos(requisitos) {
            var _this6 = this;

            var requisito = Object.values(requisitos);
            var parsed = [];
            requisito.forEach(function (valor, index) {
                var obj = {};
                if (valor.tipo == 'auto') {
                    var email = {};
                    if (Number.isInteger(valor.opciones.tipo_tarea)) {
                        email = _this6.tipo_tarea_email.filter(function (tarea_email) {
                            return tarea_email.id === parseInt(valor.opciones.tipo_tarea);
                        })[0];
                    } else {
                        email = {
                            id: _this6.tipo_tarea_email[0].id,
                    ...
                    if (typeof valor.opciones.tipo_fecha !== 'undefined') {
                        fecha = _this6.tipo_fecha_auto.filter(function (fecha_auto) {
                            return fecha_auto.tipo === valor.opciones.tipo_fecha;
                        })[0];
                    } else {
                        fecha = {
