...
new __WEBPACK_IMPORTED_MODULE_0_vue___default.a({
	...
	data: {
		form: {
			...
			proceso: '',
			estado_posterior: [],
			requisitos: [{ nombre: '', tipo: '', opciones: [] }],
			titulo_tarea: '',
			tiempo_seguimiento: ''
		},
	...
	methods: {
		getData: function getData() {
			...
			__WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('api.procesos')).then(function (res) {
				res.data.procesos.forEach(function (proceso) {
					var obj = {
						nombre: proceso.nombre,
						id: proceso.id
					};
					_this.procesos.push(obj);
				});
			});
			__WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('api.correos')).then(function (res) {
				_this.tipo_tarea_email = res.data.correos;
			});
		...
		agregarRequisito: function agregarRequisito(index) {
		...
		store: function store() {
			...
			__WEBPACK_IMPORTED_MODULE_1_axios___default.a.post(laroute.route('estados.store'), this.form).then(function (res) {
				if (res.data.saved) {
					__WEBPACK_IMPORTED_MODULE_4_sweetalert2___default()('El estado se ha guardado con éxito!', '', 'success');
					setTimeout(function () {
						window.location = laroute.route('procesos.show', { proceso: _this2.form.proceso.id });
					}, 1500);
					return;
				}
			}).catch(function (error) {
		...
		getEstados: function getEstados() {
			...
			__WEBPACK_IMPORTED_MODULE_1_axios___default.a.get(laroute.route('api.estados')).then(function (res) {
				res.data.estados.forEach(function (estado) {
					if (_this3.form.proceso.id == estado.proceso_id) {
						var obj = {
