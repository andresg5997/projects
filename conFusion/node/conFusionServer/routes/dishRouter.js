...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
	res.statusCode = 403;
	res.end('POST not supported in /dishes/' + req.params.dishId + '/comments/' + req.params.commentId);
})
.put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
	Dishes.findById(req.params.dishId)
	.then((dish) => {
		if(dish != null && dish.comments.id(req.params.commentId) != null) {
			// Added toString() to make able the comparison
			if(req.user._id.toString() === dish.comments.id(req.params.commentId).author.toString()) {
				...
				var err = new Error('You are not the author of the comment!');
				err.status = 403;
				next(err);
			}
		} else if(dish == null) {
