...
class AffiliateAudioViewController extends AffiliateController
{
    public function create(Request $request, $slug, $embed)
    {
        ...
        $commissions_audio = $user->commissions_audio;
        $adblock = $request->input('test');
        $audio_multiplier = config('audio_multiplier');
        ...
        $commission = $amount_per_1_view * $ads_multiplier * $adblock_multiplier * $audio_multiplier * 0.1;
        $new_commissions_audio = $commissions_audio + $commission;
        ...
        $affiliate = new AffiliateAudioView();
        if ($affiliate->where('ip', $ip)
            ...
            $user->where('id', $owner_id)->update([
                ...
                'commissions_audio'         => $new_commissions_audio,
    ...
    public function statistics(Request $request)
    {
        ...
        if (Cache::has($yesterdays_viewsFromTo_key)) {
            ...
            for ($i = 0; $i < 12; $i++) {
                if ($i == 0) {
                    $yesterdays_viewsFromTo[$i] = AffiliateAudioView::where('owner_id', $user->id)
                        ->where('created_at', '>=', Carbon::yesterday()->startOfDay())
                        ->where('created_at', '<=', Carbon::yesterday()->addHour(2))
                        ->count();

                    $todays_viewsFromTo[$i] = AffiliateAudioView::where('owner_id', $user->id)
        ...
        if (Cache::has($past_views_key)) {
            ...
            $past_views = [];
            for ($i = 0; $i < 30; $i++) {
                $past_views[$i] = AffiliateAudioView::where('owner_id', $user->id)
            ...
            if ($diff_in_days >= 1 && $diff_in_days < 30) {
                ...
                for ($day = 0; $day < $diff_in_days; $day++) {
                    for ($i = 1; $i <= 4; $i++) {
                        ...
                        for ($x = 0.1; $x <= 1; $x += 0.1) {
                            ...
                            for ($z = 10; $z <= 100; $z += 10) {
                                $views_last_30_days[$day][$i][(int) $multiplier][$z] = AffiliateAudioView::where('owner_id', $user->id)
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    for ($x = 0.1; $x <= 1; $x += 0.1) {
                        ...
                        $multiplier = round($x * 100);
                        for ($z = 10; $z <= 100; $z += 10) {
                            $views_last_30_days[0][$i][(int) $multiplier][$z] = AffiliateAudioView::where('owner_id', $user->id)
                                ->where('percent_played', $z)
                                ->where('country_group', $i)
        ...
        if (Cache::has($this_years_views_key)) {
            ...
            $this_years_views = AffiliateAudioView::where('owner_id', $user->id)
        ...
        if (Cache::has($percentages_watched_key)) {
            ...
            for ($x = 10; $x <= 100; $x++) {
                for ($i = 0; $i < 30; $i++) {
                    $percentages_watched[$x][$i] = AffiliateAudioView::where('owner_id', $user->id)
                        ->where('created_at', '>=', Carbon::today()->subDays($i)->startOfDay())
                        ->where('created_at', '<=', Carbon::today()->subDays($i)->endOfDay())
        ...
        if (Cache::has($todays_percentage_views_key)) {
            ...
            for ($i = 10; $i <= 100; $i += 10) {
                $todays_percentage_views[$i] = AffiliateAudioView::where('owner_id', $user->id)
                    ->where('created_at', '>=', Carbon::today()->startOfDay())
        ...
        if (Cache::has($yesterdays_percentage_views_key)) {
            ...
            for ($i = 10; $i <= 100; $i += 10) {
                $yesterdays_percentage_views[$i] = AffiliateAudioView::where('owner_id', $user->id)
                    ->where('created_at', '>=', Carbon::yesterday()->startOfDay())
        ...
        if (Cache::has($last_7_days_percentage_views_key)) {
            ...
            for ($i = 10; $i < 101; $i += 10) {
                $last_7_days_percentage_views[$i] = AffiliateAudioView::where('owner_id', $user->id)
        ...
        if (Cache::has($last_30_days_percentage_views_key)) {
            ...
            for ($i = 10; $i < 101; $i += 10) {
                $last_30_days_percentage_views[$i] = AffiliateAudioView::where('owner_id', $user->id)
        ...
        if (Cache::has($this_years_percentage_views_key)) {
            ...
            for ($i = 10; $i < 101; $i += 10) {
                $this_years_percentage_views[$i] = AffiliateAudioView::where('owner_id', $user->id)
                    ->where('created_at', '>=', Carbon::parse('first day of January'))
                    ->where('created_at', '<=', Carbon::parse('first day of January next year')->subSecond())
                    ->where('percent_played', $i)
                    ->count();
        ...
        if (Cache::has($this_months_revenue_key)) {
            ...
            if ($diff_in_days >= 1 && $diff_in_days < 30) {
                ...
                for ($i = 0; $i < $diff_in_days; $i++) {
                    $this_months_revenue[$i] = array_sum(AffiliateAudioView::where('owner_id', $user->id)
                        ->where('created_at', '>=', Carbon::today()->subDays($i)->startOfDay())
                        ->where('created_at', '<=', Carbon::today()->subDays($i)->endOfDay())
                        ->pluck('commission')
                        ->toArray());
                ...
                // echo "update because $diff_in_minutes >= $expires_at_interval";
                // add today's stats to the array again to get an updated array
                $this_months_revenue[0] = array_sum(AffiliateAudioView::where('owner_id', $user->id)
                    ->where('created_at', '>=', Carbon::today()->subDays(0)->startOfDay())
                    ->where('created_at', '<=', Carbon::today()->subDays(0)->endOfDay())
                    ->pluck('commission')
                    ->toArray());
        ...
        $this_years_audio_revenue = $this->trimTrailingZeroes($user->commissions_audio);
        ...
        if (Cache::has($adblock_usage_yes_key)) {
            ...
            $adblock_usage_yes = AffiliateAudioView::where('owner_id', $user->id)
                ->where('adblock', 1)
                ->count();

            $adblock_usage_no = AffiliateAudioView::where('owner_id', $user->id)
                ->where('adblock', 0)
                ->count();
