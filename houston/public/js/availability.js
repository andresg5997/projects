...
new __WEBPACK_IMPORTED_MODULE_0_vue___default.a(_defineProperty({
    el: '#app',
    data: {
    ...
    methods: {
        isParent: function isParent(player) {
            this.user_email == player.email ? true : false;
        },
        markAllGames: function markAllGames(player) {
            var _this = this;

            this.events.forEach(function (event) {
                _this.changeStatus(player, event.id, 3, false);
            });
            this.getData();
            toastr.success('Player marked as available for all games!');
        },
        changeStatus: function changeStatus(player, eventId, status) {
            var _this2 = this;

            var notify = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : true;

            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.post('availability', { player_id: player.id, status: parseInt(status), event_id: eventId }).then(function (res) {
                var availability = _this2.availability.filter(function (availability) {
                ...
                if (availability.length > 0) {
                    ...
                    _this2.availability.push({ player_id: player.id, status: parseInt(status) });
                }
                _this2.getData();
                if (notify) {
        ...
        isAvailable: function isAvailable(player) {
            var availability = this.availability.filter(function (availability) {
                if (availability.player_id === player.id) {
                    // console.log(availability)
                    return availability;
                }
        ...
        getPlayers: function getPlayers() {
            ...
            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get('/teams/' + this.form.team_id + '/ajax').then(function (res) {
        ...
        getData: function getData() {
            ...
            __WEBPACK_IMPORTED_MODULE_1_axios___default.a.get('/teams/' + this.form.team_id + '/getAvailability').then(function (res) {
                // console.log('qbta')
                _this4.availability = res.data.availability;
                _this4.event = res.data.event;
                _this4.events = res.data.events;
            }).catch(function (error) {
                return console.log(error);
            });
        },
        eventClass: function eventClass(player, event) {
        ...
        eventStatus: function eventStatus(player, event) {
            var availability = event.availability.filter(function (availability) {
                if (availability.player_id === player.id) {
                    // console.log(availability)
                    return availability;
                }
