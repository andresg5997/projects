...
export const survivalRule = (x, y, matrix, [isAlive, color = BLUE]) => {
    ...
    return [isCurrentCellAlive(cell, neighbours), color];
};

/**
 * Local function to check if selected cell remais alive
 * 
 * @param {number} neighbours
 * @param {array} cell
 *
 */
const isCurrentCellAlive = (cell, neighbours) => {
    let currentCellisAlive = false;
    if(
        (isCellAlive(cell) && neighbours === 2) ||
        (isCellAlive(cell) && neighbours === 3) ||
        (!isCellAlive(cell) && neighbours === 3)
    ){
...
export const getMostFrequentColor = neighbourColors => {
    let colorOcurrencies = neighbourColors.reduce((acc, curr) => { 
        if (curr in acc)
            acc[curr]++;
        else
            acc[curr] = 1;
    ...
    let mostCommonColor = parseInt(Object.keys(colorOcurrencies).reduce((a, b) => colorOcurrencies[a] > colorOcurrencies[b] ? a : b));
